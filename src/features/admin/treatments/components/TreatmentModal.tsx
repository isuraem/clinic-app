...
import { saveTreatment } from '../treatmentsSlice'
import type { Treatment, PricingType } from '@/types'
import Modal from '@/components/ui/Modal'
import Button from '@/components/ui/Button'
import Input from '@/components/ui/Input'

interface Props { open: boolean; onClose: () => void; treatment: Treatment | null }
...
export default function TreatmentModal({ open, onClose, treatment }: Props) {
  ...
  useEffect(() => {
    if (treatment) {
      reset({
        ...treatment,
        variations: treatment.variations ?? [{ label: '', price: '', memberPrice: '' }],
        bundles:    treatment.bundles    ?? [{ sessions: 3, price: '', label: '' }],
        expectationCards: treatment.expectationCards ?? [],
      })
    } else {
      reset({ pricingType: 'individual', variations: [{ label: '', price: '' }], bundles: [{ sessions: 3, price: '', label: 'Starter Pack' }], expectationCards: [] })
    }
  }, [treatment, reset])

  const onSubmit = async (data: any) => {
    await dispatch(saveTreatment({ ...data, id: treatment?.id, clinicId: 'clinic-demo', galleryImages: [], beforeAfterImages: [] }))
    onClose()
  }

  return (
    <Modal open={open} onClose={onClose} title={treatment ? 'Edit Treatment' : 'New Treatment'} size="2xl">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 max-h-[75vh] overflow-y-auto pr-1">

        {/* Basic info */}
        <div className="grid grid-cols-2 gap-4">
          <Input label="Treatment Name" placeholder="HydraFacial" error={errors.name?.message as string} {...register('name', { required: 'Required' })} />
          <div>
        ...
        <label className="flex items-center gap-3 cursor-pointer">
          ...
          <span className="text-sm text-gray-700 font-medium">Members-only treatment</span>
        </label>

        <div className="flex justify-end gap-3 pt-2">
          <Button type="button" variant="secondary" onClick={onClose}>Cancel</Button>
          <Button type="submit" loading={isSubmitting}>{treatment ? 'Save Changes' : 'Create Treatment'}</Button>
        </div>
      </form>
    </Modal>
  )
